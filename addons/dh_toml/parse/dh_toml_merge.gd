...
## LIST 决定当前根路径
## 需要保存注册过的路径来放置冲突
func merge(_token: DHTomlTokenObject) -> void:
	...
	if _token.type == DHTomlTokenObject.TOKEN_TYPE.LIST:
		...
		if root_is_array:
			...
			curr_load.append(target_load[0])
